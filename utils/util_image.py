...
def calculate_ms_ssim(im1, im2, border=0, ycbcr=False, weights=None):
    '''
    MS-SSIM metric.
    im1, im2: h x w x , [0, 255], uint8
    weights: List of weights for different scales (default uses the recommended values).
    '''
    if not im1.shape == im2.shape:
        raise ValueError('Input images must have the same dimensions.')
    ...
    if im1.ndim == 2:
        return _ms_ssim(im1, im2, weights)
    elif im1.ndim == 3:
        if im1.shape[2] == 3:
            ms_ssims = []
            for i in range(3):
                ms_ssims.append(_ms_ssim(im1[:, :, i], im2[:, :, i], weights))
            return np.array(ms_ssims).mean()
        elif im1.shape[2] == 1:
            return _ms_ssim(np.squeeze(im1), np.squeeze(im2), weights)
    else:
        raise ValueError('Wrong input image dimensions.')

def _ms_ssim(img1, img2, weights):
    '''
    Helper function to calculate MS-SSIM for grayscale images
    '''
    img1 = img1.astype(np.float64)
    img2 = img2.astype(np.float64)
    ...
    for _ in range(levels):
        ...
        # SSIM and contrast-structure (CS) maps
    ...
    # Combine SSIM and CS values across scales
    mssim = np.array(mssim)
    mcs = np.array(mcs)
    ms_ssim_value = (np.prod(mcs[:-1]**weights[:-1]) * (mssim[-1]**weights[-1]))
...
def batch_MS_SSIM(img, imclean, border=0, ycbcr=False, weights=None):
    '''
    Batch processing for MS-SSIM.
    '''
    if ycbcr:
        img = rgb2ycbcrTorch(img, True)
        imclean = rgb2ycbcrTorch(imclean, True)
    ...
    MS_SSIM = 0
    for i in range(Img.shape[0]):
        MS_SSIM += calculate_ms_ssim(Iclean[i, :, ].transpose((1, 2, 0)), Img[i, :, ].transpose((1, 2, 0)), border, weights)
    return MS_SSIM
