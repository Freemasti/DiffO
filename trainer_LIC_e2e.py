...
import lpips
from models.lpips_stable import NormFixLPIPS
import numpy as np
...
class TrainerBase:
    ...
    def build_model(self):
        ...
        if self.configs.lic.target is not None:
            lic_params = self.configs.lic.get('params', dict)
            lic=util_common.get_obj_from_str(self.configs.lic.target)(**lic_params)
            # if self.num_gpus > 1:
            #     self.lic = DDP(lic.cuda(), device_ids=[self.rank,], broadcast_buffers=False)  # wrap the network
            # else:
            #    self.lic = lic.cuda()
            self.lic = lic.cuda()
        # model information
    ...
    def save_ckpt(self):
        if self.rank == 0:
            ...
            if self.configs.autoencoder.target is not None:
                autoencoder_ckpt_path = self.ckpt_dir / 'vq_model_{:d}.pth.tar'.format(self.current_iters)
                torch.save({'iters_start': self.current_iters,
                'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                'state_dict': self.autoencoder.state_dict()}, autoencoder_ckpt_path)
            if self.configs.GT_encoder.target is not None:
                GT_encoder_ckpt_path = self.ckpt_dir / 'GT_vq_model_{:d}.pth.tar'.format(self.current_iters)
                torch.save({'iters_start': self.current_iters,
                'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                'state_dict': self.GT_encoder.state_dict()}, GT_encoder_ckpt_path)
...
class TrainerDifIR(TrainerBase):
    ...
    def setup_optimizaton(self):
        if self.configs.train.train_hyp_dec:
            self.optimizer = torch.optim.AdamW([
                    {'params':self.autoencoder.parameters(), 'lr':self.configs.train.lr},
                    {'params':self.GT_encoder.parameters(), 'lr':self.configs.train.lr}
                    ],
                    lr=self.configs.train.lr,
                    weight_decay=self.configs.train.weight_decay)
        else:
            self.optimizer = torch.optim.AdamW([
                        {'params':self.autoencoder.encoder.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.autoencoder.decoder.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.autoencoder.quant_conv.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.autoencoder.post_quant_conv.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.autoencoder.quantize.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.model.parameters()},
                        #{'params':self.GT_encoder.module.decoder.parameters() if self.num_gpus > 1 else self.GT_encoder.decoder.parameters(), 'lr':self.configs.train.lr}
                        {'params':self.GT_encoder.module.encoder.parameters() if self.num_gpus > 1 else self.GT_encoder.encoder.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.GT_encoder.module.decoder.parameters() if self.num_gpus > 1 else self.GT_encoder.decoder.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.GT_encoder.module.qunat_conv.parameters() if self.num_gpus > 1 else self.GT_encoder.quant_conv.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.GT_encoder.module.quantize.parameters() if self.num_gpus > 1 else self.GT_encoder.quantize.parameters(), 'lr':self.configs.train.lr},
                        {'params':self.GT_encoder.module.post_quant_conv.parameters() if self.num_gpus > 1 else self.GT_encoder.post_quant_conv.parameters(), 'lr':self.configs.train.lr},
                        ],
                        lr=self.configs.train.lr,
                        weight_decay=self.configs.train.weight_decay)
    ...
    def resume_from_ckpt(self):
        ...
        if self.configs.resume:
            ...
            if self.rank == 0:
                self.logger.info(f"=> Loaded checkpoint from {self.configs.resume}")
            ckpt = torch.load(self.configs.resume, map_location=f"cuda:{self.rank}")
            autoencoder_ckpt = torch.load(self.configs.resume.replace("model", "vq_model") + ".tar", map_location=f"cuda:{self.rank}")
            util_net.reload_model(self.model, ckpt['state_dict'])
            util_net.reload_model(self.autoencoder, autoencoder_ckpt['state_dict'])
            ...
            self.iters_start = 0
    def build_model(self):
        self.lpips_loss = NormFixLPIPS(net='vgg').eval().cuda()
        params = self.configs.model.get('params', dict)
        model = util_common.get_obj_from_str(self.configs.model.target)(**params)

        if self.configs.train.train_model == False:
        ...
        if self.configs.autoencoder is not None:
            ...
            if self.num_gpus > 1:
                self.autoencoder = DDP(autoencoder.cuda(), device_ids=[self.rank,], broadcast_buffers=False)  # wrap the network
            else:
            ...
            if self.configs.autoencoder.ckpt_path is not None:
                ...
                if 'state_dict' in ckpt:
                    ...
                    ckpt = {k: v for k, v in ckpt.items() if not k.startswith("external.")}
                    ckpt = {k: v for k, v in ckpt.items() if not k.startswith("loss.")}
                    #print(ckpt.keys())
                    util_net.reload_model(self.autoencoder, ckpt)

                else:
                    autoencoder.load_state_dict(ckpt, True)
                # GT_encoder ckpt가 제공되었다면 해당 파라미터만 별도로 로드합니다.
                if hasattr(self.configs.autoencoder, 'gt_encoder_ckpt_path') and self.configs.autoencoder.gt_encoder_ckpt_path is not None:
                    gt_ckpt = torch.load(self.configs.autoencoder.gt_encoder_ckpt_path, map_location=f"cuda:{self.rank}")
                    if 'state_dict' in gt_ckpt:
                        gt_ckpt = gt_ckpt['state_dict']
                    gt_keys = {k: v for k, v in gt_ckpt.items() 
                            if k.startswith("quantize.") or k.startswith("quant_conv.") or k.startswith("post_quant_conv.")}

                    # strict=False로 업데이트하여 기존 파라미터와 병합합니다.
                    self.autoencoder.load_state_dict(gt_keys, strict=False)

            else: 
                if self.rank == 0:
                    self.logger.info(f"Autoencoder training starts from scratch")
            
            if self.configs.train.train_autoencoder != True:
                #for params in autoencoder.parameters():
                    #params.requires_grad_(False)
                autoencoder.eval()
            # else :
            #     for params in autoencoder.quantize.parameters():
            #         params.requires_grad_(False)
            #     for params in autoencoder.quant_conv.parameters():
            #         params.requires_grad_(False)
            #     for params in autoencoder.post_quant_conv.parameters():
            #         params.requires_grad_(False)
        ...
        # GT_encoder
        if self.configs.GT_encoder is not None:
            params = self.configs.GT_encoder.get('params', dict)
            GT_encoder = util_common.get_obj_from_str(self.configs.GT_encoder.target)(**params)
            if self.num_gpus > 1:
                self.GT_encoder = DDP(GT_encoder.cuda(), device_ids=[self.rank,], broadcast_buffers=False)  # wrap the network
            else:
                self.GT_encoder = GT_encoder.cuda()
            if self.configs.GT_encoder.ckpt_path is not None:
                GT_ckpt = torch.load(self.configs.GT_encoder.ckpt_path, map_location=f"cuda:{self.rank}")
                if self.rank == 0:
                    self.logger.info(f"Restoring GT_encoder from {self.configs.GT_encoder.ckpt_path}")
                if 'state_dict' in GT_ckpt:
                    GT_ckpt = GT_ckpt['state_dict']

                    #GT_ckpt = {k[len("external."):]: v for k, v in GT_ckpt.items() if k.startswith("external.")}
                    GT_ckpt = {k: v for k, v in GT_ckpt.items() if not k.startswith("loss.")}



                    util_net.reload_model(self.GT_encoder, GT_ckpt)
                else:
                    GT_encoder.load_state_dict(GT_ckpt, True)
            else: 
                if self.rank == 0:
                    self.logger.info(f"GT_encoder training starts from scratch")
            
            if self.configs.train.train_GT_encoder != True:
                for params in GT_encoder.encoder.parameters():
                    params.requires_grad_(False)
                for params in GT_encoder.decoder.parameters():
                    params.requires_grad_(False)
                for params in GT_encoder.quant_conv.parameters():
                    params.requires_grad_(False)
                for params in GT_encoder.quantize.parameters():
                    params.requires_grad_(False)
                for params in GT_encoder.post_quant_conv.parameters():
                    params.requires_grad_(False)
                #GT_encoder.eval()
            else :
                for params in GT_encoder.parameters():
                    params.requires_grad_(True)
                GT_encoder.train()
            if self.configs.GT_encoder.use_fp16:
                self.GT_encoder = GT_encoder.half().cuda()
            # else:
            #     self.autoencoder = autoencoder.cuda()
        else:
    ...
    @torch.no_grad()
    def prepare_data(self, data, dtype=torch.float32, realesrgan=None, phase='train', LIC_model=None):
        ...
        if realesrgan and phase == 'train':
            ...
            # blur
            if self.configs.degradation['LIC_model'] :
                #mode = random.choice(['area', 'bilinear', 'bicubic'])
                mode = 'bicubic'
                out = F.interpolate(
                    im_gt,
                    size=(int(ori_h // sf), int(ori_w // sf)),
                    mode=mode,
                    )
                if self.configs.degradation['LIC_model'] != 'E2E':
                    out = LIC.recon(self.configs.degradation['LIC_model'], self.configs.degradation['LIC_quality'], out).cuda()
                # else:
                #     out = LIC_model(out)["x_hat"]
            else:
            ...
            return {
                key: value.cuda().to(dtype=dtype) 
                for key, value in data.items() 
                if value.size(-1) >= 256 and value.size(-2) >= 256  # 이미지 크기 필터링
            }
    ...
    def train(self):
        ...
        self.model.train()
        # if self.configs.train.train_autoencoder:
        #     self.autoencoder.train()
        # if self.configs.train.train_GT_encoder:
        #     self.GT_encoder.train()
        num_iters_epoch = math.ceil(len(self.datasets['train']) / self.configs.train.batch[0])
        for ii in range(self.iters_start, self.configs.train.iterations):
            self.current_iters = ii + 1
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            latent_downsamping_sf = 2**(len(self.configs.GT_encoder.params.ddconfig.ch_mult) - 1) if self.configs.GT_encoder is not None else 1
            ...
            compute_losses = functools.partial(
                self.base_diffusion.training_losses if not self.configs.train.train_hyp_dec else self.base_diffusion.training_losses_hyp_vq,
                self.model,
                micro_data['gt'],
                micro_data['lq'],
                tt,
                ...
                encode_lq=self.configs.train.encode_lq,
                only_use_yhat=self.configs.train.only_use_yhat,
                train_hyp_dec=self.configs.train.train_hyp_dec,
                noise=noise,
                lpips_loss=self.lpips_loss,
                distortion_weight=self.configs.train.distortion_weight,
                perceptual_weight=self.configs.train.perceptual_weight,
                loss_in_image_space=self.configs.train.loss_in_image_space,
                resshift=self.configs.train.resshift,
                add_noise_to_condition=self.configs.train.add_noise_to_condition,
            )
            if self.configs.train.use_fp16:
                with amp.autocast():
    ...
    def validation(self, phase='val'):
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                    ...
                    if (num_iters + 1) in indices or num_iters + 1 == 1:
                        for key, value in sample.items():
                            if key in ['sample', 'pred_xstart']:
                                if self.configs.train.train_hyp_dec:
                                    sample_encode[key] = self.base_diffusion.encode_first_stage(
                                        im_lq, # 难道这里要改
                                        self.GT_encoder if self.GT_encoder is not None else self.autoencoder,
                                        force_not_quantize=False,
                                    )
                                    sample_encode[key] = self.base_diffusion.encode_first_stage(
                                        sample_encode[key], # 难道这里要改
                                        self.autoencoder,
                                    ...
                                    sample_decode[key] = self.base_diffusion.decode_first_stage(
                                        sample_encode[key], # 难道这里要改
                                        self.autoencoder,
                                        encoder_up_sample=False,
                                        force_not_quantize=False,
                                    )
                                    sample_decode[key] = self.base_diffusion.decode_first_stage(
                                        sample_decode[key], # 难道这里要改
                                        self.GT_encoder,
                                        encoder_up_sample=False,
                                        force_not_quantize=True,
                                    )
                ...
                with torch.no_grad():
                    results = sample_decode['sample'].detach().clamp(0, 1)
                ...
                if True: #if 'gt' in data:
                    ...
                    mean_lpips += self.lpips_loss(sample_decode['sample'].detach(), im_lq).sum().item()
                ...
                if (ii + 1) % self.configs.train.log_freq[2] == 0:
                    ...
                    if True: #if 'gt' in data:
                        x4 = vutils.make_grid(im_lq, normalize=True)
...
class TrainerDistillDifIR(TrainerDifIR):
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            latent_downsamping_sf = 2**(len(self.configs.GT_encoder.params.ddconfig.ch_mult) - 1)
            latent_resolution = micro_data['gt'].shape[-1] // latent_downsamping_sf
            noise = torch.randn(
                    size=micro_data['gt'].shape[:2] + (latent_resolution, ) * 2,
                    device=micro_data['gt'].device,
...
class TrainerDistillDifIR_LIC_E2E(TrainerDistillDifIR):
    def __init__(self, configs):
        super().__init__(configs)
        self.distill_ddpm = configs.train.get("distill_ddpm", False)
        self.uncertainty_hyper = configs.train.get("uncertainty_hyper", False)
    ...
    def setup_optimizaton(self):
        # self.optimizer = torch.optim.AdamW( list(self.lic.parameters())+list(self.model.parameters()), # lic+model e2e
        #                                     lr=self.configs.train.lr,
        #                                     weight_decay=self.configs.train.weight_decay)
        # 필터링된 파라미터 분리
        aux_params = []
        ...
        # for name, param in self.lic.named_parameters():
        #     if  param.requires_grad and "quantiles" in name:
        #         aux_params.append(param)
        #     elif  param.requires_grad and "quantiles" not in name:
        #         net_params.append(param)

        # 두 개의 옵티마이저 생성
        # self.aux_optimizer = torch.optim.AdamW(aux_params, 
        #                                             lr=self.configs.train.lr * 0.1,  # 예시로 더 작은 lr 설정
        #                                             weight_decay=self.configs.train.weight_decay)

        if self.configs.train.train_first_stage_model:
            self.optimizer = torch.optim.AdamW(list(net_params) + list(self.model.parameters()) + list(self.autoencoder.parameters()), 
                                    lr=self.configs.train.lr,
                                    weight_decay=self.configs.train.weight_decay)
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min',  patience=400)
        else:
            self.optimizer = torch.optim.AdamW(list(net_params) + list(self.model.parameters()), 
                                            lr=self.configs.train.lr,
                                            weight_decay=self.configs.train.weight_decay)
    ...
    def build_model(self):
        ...
        # LIC
        if self.configs.lic is not None:
            if self.rank == 0:
                self.logger.info(f"Restoring LIC from {self.configs.lic.ckpt_path}")
            
            lic_params = self.configs.lic.get('params', dict)
            lic = util_common.get_obj_from_str(self.configs.lic.target)(**lic_params)
            
            # # Load checkpoint
            # if self.configs.lic.ckpt_path:
            #     lic_ckpt = torch.load(self.configs.lic.ckpt_path, map_location=f"cuda:{self.rank}")
                
            #     lic.from_state_dict(lic_ckpt)
            
            # Enable training mode
            #lic.train()
            lic.requires_grad_(False)
            lic.eval()
            
            # # Ensure gradients are enabled --> 이거때문에 오류? (RuntimeError: Expected to have finished reduction in the prior iteration before starting a new one.)
            # for param in lic.parameters():
            #     param.requires_grad_(True)

            # if self.num_gpus > 1:
            #     self.lic = DDP(lic.cuda(), device_ids=[self.rank,], broadcast_buffers=False, find_unused_parameters=True)  # wrap the network
            # else:
            #     self.lic = lic.cuda()
            ...
            # Define optimizer
            #learning_rate = self.configs.lic.get('learning_rate', 1e-4)
            #self.lic_optimizer = torch.optim.Adam(self.lic.parameters(), lr=learning_rate)
        ...
        if self.configs.autoencoder is not None:
            ...
            if self.num_gpus > 1 and self.configs.train.train_first_stage_model==True:
                self.autoencoder = DDP(autoencoder.cuda(), device_ids=[self.rank,], broadcast_buffers=False, find_unused_parameters=True)  # wrap the network
    ...
    def training_step(self, data):
        ...
        self.optimizer.zero_grad()
        #self.aux_optimizer.zero_grad() ##auxillary loss optimizer
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            compute_losses = functools.partial(
                self.base_diffusion.training_losses_RD,
                self.model,
                self.teacher_model,
                micro_data['gt'], # image range 0-1
                micro_data['lq'],
                ...
                LIC_model = self.lic,
                first_stage_model=self.autoencoder,
                train_first_stage_model=self.configs.train.train_first_stage_model,
                model_kwargs=model_kwargs,
                noise=noise,
                distill_ddpm=self.distill_ddpm,
                uncertainty_hyper=self.uncertainty_hyper,
                uncertainty_num_aux=self.uncertainty_num_aux,
                learn_distill=self.configs.train.learn_distill,
                learn_xT=self.learn_xT,
                finetune_use_gt=self.finetune_use_gt,
                finetune_RP = self.configs.train.finetune_RP,
                train_lpips = self.configs.train.lpips,
                lpips_loss = self.lpips_loss,
                reformulated_reflow=self.reformulated_reflow,
                xT_cov_loss=self.xT_cov_loss,
                loss_in_image_space=self.loss_in_image_space,
                rate_weight=self.configs.train.rate_weight,
                distortion_weight=self.configs.train.distortion_weight,
                perceptual_weight=self.configs.train.perceptual_weight,
            )
            # compute_aux_losses = functools.partial(
            #     self.base_diffusion.aux_loss,
            #     LIC_model = self.lic
            # )
            if self.configs.train.use_fp16:
                with amp.autocast():
                    if last_batch or self.num_gpus <= 1:
                        ...
                        with self.model.no_sync():
                            ...
                            #aux_losses = compute_aux_losses()
                    loss = losses["loss"].mean() / num_grad_accumulate
                    #aux_loss = aux_losses.mean()/num_grad_accumulate
                scaler.scale(loss).backward()
    ...
    def validation(self, phase='val'):
        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                if self.configs.degradation.LIC_model == 'E2E':
                    im_lq = im_lq * 0.5 + 0.5 # [-1,1] --> [0,1]
                    im_lq = self.lic(im_lq)["x_hat"]
                    im_lq = (im_lq - 0.5) / 0.5 # [0,1] --> [-1,1]
                results = self.base_diffusion.ddim_sample_loop(
                    y=im_lq,
                    model=self.model,
                    first_stage_model=self.autoencoder,
    ...
    def save_ckpt(self):
        if self.rank == 0:
            ...
            torch.save({'iters_start': self.current_iters,
                        ...
                        'state_dict': self.model.state_dict()}, ckpt_path)
            if self.configs.lic.target is not None:
                lic_ckpt_path = self.ckpt_dir / 'lic_model_{:d}.pth.tar'.format(self.current_iters)
                torch.save({'iters_start': self.current_iters,
                'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                'state_dict': self.lic.state_dict()}, lic_ckpt_path)

            if self.configs.lic.target is not None:
                vqvae_ckpt_path = self.ckpt_dir / 'vqvae_model_{:d}.pth.tar'.format(self.current_iters)
                torch.save({'iters_start': self.current_iters,
                'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                'state_dict': self.autoencoder.state_dict()}, vqvae_ckpt_path)

            if hasattr(self, 'ema_rate'):
                ema_ckpt_path = self.ema_ckpt_dir / 'ema_model_{:d}.pth'.format(self.current_iters)
                torch.save(self.ema_state, ema_ckpt_path)

class TrainerDistillDifIR_LIC_E2E_CW(TrainerDistillDifIR):
    def __init__(self, configs):
        super().__init__(configs)
        self.distill_ddpm = configs.train.get("distill_ddpm", False)
        self.uncertainty_hyper = configs.train.get("uncertainty_hyper", False)
    ...
    def setup_optimizaton(self):
        ...
        if self.configs.train.train_all:
            self.optimizer = torch.optim.AdamW(list(self.vae.parameters()) + list(self.autoencoder.parameters()) + list(self.model.parameters()), 
                                            lr=self.configs.train.lr,
                                            weight_decay=self.configs.train.weight_decay)
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min',  patience=400)
        else:
            self.optimizer = torch.optim.AdamW(list(self.autoencoder.parameters()) + list(self.model.parameters()), 
                                            lr=self.configs.train.lr,
                                            weight_decay=self.configs.train.weight_decay)
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, 'min',  patience=400)
    ...
    def build_model(self):
        ...
        if self.configs.model.teacher_ckpt_path is not None:
            teacher_ckpt_path = self.configs.model.teacher_ckpt_path
            if self.rank == 0:
                self.logger.info(f"[INFO]: Initializing the teacher model from {teacher_ckpt_path}")
            ckpt = torch.load(teacher_ckpt_path, map_location=f"cuda:{self.rank}")
        ...
        # if self.configs.vae is not None:
        #     if self.configs.vae.ckpt_path.endswith(".safetensors"):
        #         ckpt = load_file(self.configs.vae.ckpt_path, device=f"cuda:{self.rank}")
        #     else:
        #         ckpt = torch.load(self.configs.vae.ckpt_path, map_location=f"cuda:{self.rank}")
        #     if self.rank == 0:
        #         self.logger.info(f"Restoring LDM-VAE from {self.configs.vae.ckpt_path}")
        #     params = self.configs.vae.get('params', dict)
        #     vae = util_common.get_obj_from_str(self.configs.vae.target)(**params)
        #     vae.load_state_dict(ckpt, True)
        #     if self.configs.train.train_vae:
        ...
        #             params.requires_grad_(False)
        #         vae.eval()
        #     if self.num_gpus > 1 and self.configs.train.train_vae==True:
        #         self.vae = DDP(vae.cuda(), device_ids=[self.rank,], broadcast_buffers=False, find_unused_parameters=True)
        #     else:
        #         if self.configs.vae.use_fp16:
        #             self.vae = vae.half().cuda()
        #         else:
        #             self.vae = vae.cuda()
        # else:
        #     self.vae = None
        
        # if self.configs.hyper_encoder is not None:
        #     if self.configs.hyper_encoder.ckpt_path.endswith(".safetensors"):
        #         ckpt = load_file(self.configs.hyper_encoder.ckpt_path, device=f"cuda:{self.rank}")
        #     else:
        #         ckpt = torch.load(self.configs.hyper_encoder.ckpt_path, map_location=f"cuda:{self.rank}")
        #     if self.rank == 0:
        #         self.logger.info(f"Restoring hyper_encoder from {self.configs.hyper_encoder.ckpt_path}")
        #     params = self.configs.hyper_encoder.get('params', dict)
        #     hyper_encoder = util_common.get_obj_from_str(self.configs.hyper_encoder.target)(**params)   
        #     hyper_encoder.load_state_dict(ckpt, True)
        #     if self.configs.train.train_hyper_encoder:
        #         for params in hyper_encoder.parameters():
        #             params.requires_grad_(True)
        #         hyper_encoder.train()
        #     else:
        #         for params in hyper_encoder.parameters():
        #             params.requires_grad_(False)
        #         hyper_encoder.eval()
        #     if self.num_gpus > 1 and self.configs.train.train_hyper_encoder==True:
        #         self.hyper_encoder = DDP(hyper_encoder.cuda(), device_ids=[self.rank,], broadcast_buffers=False, find_unused_parameters=True)
        #     else:
        #         if self.configs.hyper_encoder.use_fp16:
        #             self.hyper_encoder = hyper_encoder.half().cuda()
        #         else:
        #             self.hyper_encoder = hyper_encoder.cuda()
        # else:
    ...
    def training_step(self, data):
        ...
        for jj in range(0, current_batchsize, micro_batchsize):
            ...
            compute_losses = functools.partial(
                self.base_diffusion.training_losses_RD_CW,
                self.model,
                self.teacher_model,
                micro_data['gt'], # image range 0-1
                micro_data['lq'],
                tt,
                ldm_vae_model = None, #self.vae,
                vq_vae_model=self.autoencoder,
                train_first_stage_model=self.configs.train.train_first_stage_model,
                model_kwargs=model_kwargs,
                noise=noise,
                distill_ddpm=self.distill_ddpm,
        ...
        if self.rank == 0:
            ...
            for ii, data in enumerate(self.dataloaders[phase]):
                ...
                results = self.base_diffusion.ddim_sample_loop(
                    y=im_lq,
                    model=self.model,
                    first_stage_model=self.vae,
                    hyper_encoder=self.autoencoder,
                    noise=None,
                    clip_denoised=(self.autoencoder is None),
                    denoised_fn=None,
                    model_kwargs=model_kwargs,
    ...
    def save_ckpt(self):
        if self.rank == 0:
            ...
            if self.configs.autoencoder.target is not None:
                autoencoder_ckpt_path = self.ckpt_dir / 'vq_vae_model_{:d}.pth.tar'.format(self.current_iters)
                torch.save({'iters_start': self.current_iters,
                'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
                'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
                'state_dict': self.autoencoder.state_dict()}, autoencoder_ckpt_path)

            # if self.configs.hyper_encoder.target is not None:
            #     vae_ckpt_path = self.ckpt_dir / 'hyper_encoder_model_{:d}.pth.tar'.format(self.current_iters)
            #     torch.save({'iters_start': self.current_iters,
            #     'log_step': {phase:self.log_step[phase] for phase in ['train', 'val']},
            #     'log_step_img': {phase:self.log_step_img[phase] for phase in ['train', 'val']},
            #     'state_dict': self.hyper_encoder.state_dict()}, hyper_encoder_ckpt_path)
