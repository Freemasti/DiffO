...
class UNetModelSwin(nn.Module):
    ...
    def __init__(
        ...
        ch = input_ch = int(channel_mult[0] * model_channels)
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd(dims, in_channels, ch, 3, padding=1))] #[TimestepEmbedSequential(conv_nd(dims, model_channels, ch, 3, padding=1))] 
        )
        input_block_chans = [ch]
        ds = image_size
        self.conv_in_extended = nn.Conv2d(
            out_channels, in_channels-out_channels, kernel_size=3, padding=1
        )
        for level, mult in enumerate(channel_mult):
                ...
                if ds in attention_resolutions and jj==0:
                    layers.append(
                        BasicLayer(
                                ...
                                window_size=window_size if ch != 640 else final_window_size,
                                depth=swin_depth,
                                img_size=ds,
                                patch_size=1,
                                mlp_ratio=mlp_ratio,
    ...
    def forward(self, x, timesteps, lq=None):
        ...
        if lq is not None:
            ...
            # if lq.shape[2] < x.shape[2]:
            #     lq = F.interpolate(lq, scale_factor=2, mode='bicubic')
            #print(lq.shape, x.shape)
            #if lq.shape[3] == 32 or x.shape[3] ==32:
            #print(lq.shape, x.shape)
            #x = self.conv_in_extended(x)
            lq = self.conv_in_extended(lq) #channel extension
            x = th.cat([x, lq], dim=1)
        #print(x.shape)
        h = x.type(self.dtype)
        for ii, module in enumerate(self.input_blocks):
...
class UNetModelSwin_Comp(UNetModelSwin):
...
class Free_UNetModelSwin(UNetModelSwin):
    """
    U-Net with backbone factor and skip factor modifications.
...
class UNetModelSwin_wText(nn.Module):
    """
    The full UNet model with attention and timestep embedding.
    :param in_channels: channels in the input Tensor.
    :param model_channels: base channel count for the model.
    ...
    def __init__(
        ...
        self.clip_embed_dim = self.clip_model.config.hidden_size  # CLIP 임베딩 크기
    ...
    def forward(self, x, timesteps, lq=None, text=None):
        """
        Apply the model to an input batch.
        :param x: an [N x C x ...] Tensor of inputs.
        :param timesteps: a 1-D batch of timesteps.
        :param lq: an [N x C x ...] Tensor of low quality iamge.
        :param text: List of strings (text conditioning inputs).
        :return: an [N x C x ...] Tensor of outputs.
        """
        ...
        if text is not None:
            # 텍스트 임베딩 생성 및 추가
            text_emb = self.get_text_embedding(text).to(x.device).type(self.dtype)
            emb = emb + text_emb.unsqueeze(1)  # Embedding 차원에 텍스트 정보 추가
    ...
    def get_text_embedding(self, text):
        ...
        :param text: List of strings (batch of text inputs).
        :return: Tensor of text embeddings.
        """
        inputs = self.clip_tokenizer(text, return_tensors="pt", padding=True, truncation=True)
        with th.no_grad():
            text_embeddings = self.clip_model(**inputs).last_hidden_state  # Use the final hidden state
        return text_embeddings.mean(dim=1)  # Aggregate token embeddings (e.g., mean pooling)
