...
class BaseSampler:
    ...
    def build_model(self):
        ...
        if self.configs.GT_encoder is not None:
            ckpt_path = self.configs.GT_encoder.ckpt_path
            assert ckpt_path is not None
            self.write_log(f'Loading AutoEncoder model from {ckpt_path}...')
            GT_encoder = util_common.instantiate_from_config(self.configs.GT_encoder).cuda()
            self.load_model(GT_encoder, ckpt_path)
            GT_encoder.eval()
            if self.configs.autoencoder.use_fp16:
                self.GT_encoder = GT_encoder.half()
            else:
